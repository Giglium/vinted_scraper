...
SUPER_LINTER_FMT := -e VALIDATE_PYTHON_MYPY=false -e FIX_JAVASCRIPT_PRETTIER=true -e FIX_MARKDOWN=true -e FIX_MARKDOWN_PRETTIER=true -e FIX_JSON=true -e FIX_JSON_PRETTIER=true -e FIX_YAML_PRETTIER=true -e FIX_PYTHON_BLACK=true -e FIX_PYTHON_ISORT=true -e FIX_PYTHON_RUFF=true -e FIX_GITHUB_ACTIONS_ZIZMOR=true -e VALIDATE_TRIVY=false
...
lint: ## Run the static analysis tool to scan the codebase
	@docker run --rm --name=vinter-scraper-linter -e SHELL=/bin/bash -e IGNORE_GITIGNORED_FILES=true -e RUN_LOCAL=true -e DEFAULT_BRANCH=main $(SUPER_LINTER_FMT) --mount type=bind,src=$(ROOT),dst=/tmp/lint/ --mount type=volume,dst=/tmp/lint/.venv --platform linux/amd64 ghcr.io/super-linter/super-linter:v8.1.0
