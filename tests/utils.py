# pylint: disable=missing-module-docstring,invalid-name,missing-class-docstring,line-too-long,fixme,import-error
import json
import os
...
def _read_data_from_file(filename: str) -> Dict:
    ...
    with open(
        os.path.join(root_dir, "tests", "samples", f"{filename}.json"),
        "r",
        encoding="utf-8",
    ) as file:
